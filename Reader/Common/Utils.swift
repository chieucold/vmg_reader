...
public func asn1Length(_ data : [UInt8]) throws -> (Int, Int)  {
    ...
    throw NfcEIdReaderError.CannotDecodeASN1Length
...
public func calcHash( data: [UInt8], hashAlgorithm: String ) throws -> [UInt8] {
    ...
    if hashAlgorithm == "sha1" {
        ...
        throw NfcEIdReaderError.InvalidHashAlgorithmSpecified
    }
...
public func normalizeVietnameseAccent(_ input: String) -> String? {
    let mutableString = NSMutableString(string: input) as CFMutableString
    CFStringTransform(mutableString, nil, kCFStringTransformStripCombiningMarks, false)
    return mutableString as String
}

public func getNormalizedProvince(address: String?) -> String? {
    let strs = address?.split(separator: ",")
    if let lastStr = strs?.last {
        let lowercaseStr = lastStr.lowercased().replacingOccurrences(of: "\\s", with: "", options: .regularExpression)
        return normalizeVietnameseAccent(lowercaseStr)
    }
    return nil
}
