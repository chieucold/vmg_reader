...
public class NfcEidModel {
    ...
    public internal(set) lazy var dataGroupsPresent : [String] = {
        guard let com = dataGroupsRead[.COM] as? COM else { return [] }
        return com.dataGroupsPresent
    }()
    
    public internal(set) var dataGroupsAvailable = [DataGroupId]()
    public internal(set) var dataGroupsRead : [DataGroupId:DataGroup] = [:]
    public internal(set) var dataGroupHashes = [DataGroupId: DataGroupHash]()
    
    // --------------------------------------
    // MARK: DG1
    // --------------------------------------
    
    /// Contains the basic information such as MRZ info
    private var eidDataElements : [String:String]? {
        guard let dg1 = dataGroupsRead[.DG1] as? DataGroup1 else { return nil }
        return dg1.elements
    }
    
    public internal(set) lazy var documentType : String = { return String( eidDataElements?["5F03"]?.first ?? "?" ) }()
    public internal(set) lazy var documentSubType : String = { return String( eidDataElements?["5F03"]?.last ?? "?" ) }()
    public internal(set) lazy var documentNumber : String = { return (eidDataElements?["5A"] ?? "?").replacingOccurrences(of: "<", with: "" ) }()
    public internal(set) lazy var issuingAuthority : String = { return eidDataElements?["5F28"] ?? "?" }()
    public internal(set) lazy var documentExpiryDate : String = { return eidDataElements?["59"] ?? "?" }()
    public internal(set) lazy var dateOfBirth : String = { return eidDataElements?["5F57"] ?? "?" }()
    public internal(set) lazy var gender : String = { return eidDataElements?["5F35"] ?? "?" }()
    public internal(set) lazy var nationality : String = { return eidDataElements?["5F2C"] ?? "?" }()
    public internal(set) lazy var eidMRZ : String = { return eidDataElements?["5F1F"] ?? "NOT FOUND" }()
    
    private lazy var names : [String] = {
        return (eidDataElements?["5B"] ?? "?").components(separatedBy: "<<")
    }()
    
    public internal(set) lazy var lastName : String = {
        return names[0].replacingOccurrences(of: "<", with: " " )
    }()

    public internal(set) lazy var firstName : String = {
        var name = ""
    ...
    /// Contains the portrait image of the eID card holder - in 300 x 400 aspect ratio
    public var eidImage : UIImage? {
    ...
    /// Contains the signature image (if any).
    ...
    /// Contains additional detail information such as travel, phone number.
    public var dg11: DataGroup11? {
    ...
    /// Contains 14 fields personal additional detail information such as
    /// - eID Number
    /// - Full Name
    /// - Date Of Birth
    /// - Gender
    ...
    // --------------------------------------
    // MARK: SECURITY, CA-DG14, PA, AA-DG15
    // --------------------------------------

    public internal(set) lazy var documentSigningCertificate : X509Wrapper? = {
        return certificateSigningGroups[.documentSigningCertificate]
    }()

    public internal(set) lazy var countrySigningCertificate : X509Wrapper? = {
        return certificateSigningGroups[.issuerSigningCertificate]
    }()
    
    public internal(set) lazy var LDSVersion : String = {
        guard let com = dataGroupsRead[.COM] as? COM else { return "Unknown" }
        return com.version
    }()
    
    public internal(set) var cardAccess : CardAccess?
    public internal(set) var BACStatus : EidAuthenticationStatus = .notDone
    public internal(set) var PACEStatus : EidAuthenticationStatus = .notDone
    public internal(set) var chipAuthenticationStatus : EidAuthenticationStatus = .notDone
    public internal(set) var documentSigningCertificateVerified : Bool = false
    public internal(set) var passiveAuthenticationStatus : EidAuthenticationStatus = .notDone
    public internal(set) var activeAuthenticationStatus : EidAuthenticationStatus = .notDone
    public internal(set) var activeAuthenticationChallenge : [UInt8] = []
    public internal(set) var activeAuthenticationSignature : [UInt8] = []
    public internal(set) var verificationErrors : [Error] = []
    
    /// Indicates whether the eID has been verified with RAR.
    public var eidVerified : Bool = false
    
    /// Indicates whether the eID signature is certified.
    public var eidSignatureVerified: Bool = false

    /// Indicates if eID supports PACE.
    ...
    /// Indicates if eiD supports Chip Authentication.
    ...
    /// Indicates if eID supports Active Authentication
    ...
    private func validateAndExtractSigningCertificates( masterListURL: URL ) throws {
        ...
        self.eidVerified = true

    }
    
    private func validateTampering( useCMSVerification: Bool ) throws  {
        ...
        passiveAuthenticationStatus = .success
    }
    ...
    private func getHashesForDatagroups( hashAlgorythm: String ) -> [DataGroupId:[UInt8]]  {
    ...
    public func dumpEidData( selectedDataGroups : [DataGroupId], includeActiveAuthenticationData : Bool = false) -> [String:String] {
    ...
    /// This method performs the passive authentication in order to check if the data of eID has been tampered.
    ...
    public func verifyPassiveAuthentication( masterListURL: URL?, useCMSVerification : Bool = false ) {
        ...
        do {
            try validateTampering( useCMSVerification : useCMSVerification )
        } catch let error {
            verificationErrors.append( error )
        }
    }
    
    /// This method performs the active authentication in order to check if the eID has been cloned.
    /// - Parameter challenge: Array byte of challenge
    /// - Parameter signature: Array byte of signature
    ///  The private Key is stored in the chip's secure memory with the Public Key stored at LDS Data Group 15
    public func verifyActiveAuthentication( challenge: [UInt8], signature: [UInt8] ) {
        ...
        Log.debug( "   challenge - \(binToHexRep(challenge))")
